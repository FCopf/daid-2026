...
Os dois painéis da @fig-mesma-reta vêm da **mesma** reta. A relação sistemática é idêntica, e o que muda é o quanto as observações se afastam dela. À esquerda a relação salta aos olhos, enquanto à direita ela existe, mas exige mais dados para ser estimada com precisão.
...
O código calcula $SP_{XY}=\sum(x_i-\bar x)(y_i-\bar y)$ e $SQ_X=\sum(x_i-\bar x)^2$, obtém $\hat\beta_1=SP_{XY}/SQ_X$ e então o intercepto. Os três somatórios permitem verificar $SQ_{Total}=SQ_{Reg}+SQ_{Res}$, e a última coluna mostra que o quadrado da correlação reproduz o $R^2$ neste modelo simples com intercepto. A @fig-rikz-particao representa essa mesma partição graficamente, estação por estação.
...
À esquerda da @fig-residuos-forma, os resíduos se espalham sem padrão em torno de zero, porque a reta captou o que havia para captar. À direita, eles desenham um U nítido, sinal de que a relação não é reta, ainda que o $R^2$ possa ser alto.
...
Uma relação espécie-área é o exemplo clássico, com $S=cA^z$, em que $S$ é riqueza, $A$ é área, $c$ é a riqueza esperada quando $A=1$ e $z$ controla a velocidade de crescimento. Na escala logarítmica, o intercepto é $\log(c)$ e a inclinação é $z$, como mostra a @fig-potencia.
