...
Comparar dois grupos é uma operação bem definida, porque existe uma diferença, existe uma régua para julgá-la e existe um resultado.
...
A saída óbvia é fazer o que já se sabe fazer, três vezes, comparando o primeiro com o segundo, o primeiro com o terceiro e o segundo com o terceiro. Três testes, três valores de p, e pronto.

Essa saída tem um problema, e ele não é pequeno.

# Cada teste traz o seu próprio alarme falso

Um teste a 5% é um procedimento que, quando não há efeito nenhum, ainda assim acusa diferença em cerca de 5% das vezes. Esses 5% não são um defeito, e sim o preço combinado de antemão para poder decidir a partir de dados que variam.
...
Na @fig-inflacao, com três grupos idênticos, cerca de `r alarme_3`% dos experimentos desta simulação produziram ao menos um par "diferente". Com seis grupos, `r alarme_6`%.
...
A curva da @fig-inflacao pode ser antecipada por um cálculo simples, ao menos em um caso idealizado. Se $m$ testes fossem independentes e cada um tivesse risco $\alpha$ de alarme falso, a chance de nenhum alarme seria $(1-\alpha)^m$. Logo, a probabilidade de pelo menos um seria

$$
\operatorname{FWER}=1-(1-\alpha)^m.
$$

A sigla vem de *familywise error rate*, a taxa de erro da família de testes. Com três testes a 5%, essa conta resulta em $1-0,95^3\approx0,143$, cerca de 14,3%, valor próximo ao que a simulação produziu. A aproximação não é exata porque comparações entre as mesmas médias não são independentes, já que cada grupo participa de vários pares. A simulação preserva essa dependência e por isso estima diretamente a taxa correta.
...
Quando três testes são feitos e o resultado relatado é o mais chamativo dos três, o número informado não é o valor de p de um teste, e sim o **mínimo** de três valores de p. E mínimos de vários sorteios se concentram perto de zero mesmo quando cada sorteio isolado é bem comportado.
...
No painel superior da @fig-minimo-p, com um único teste sob hipótese nula, os valores de p se distribuem uniformemente entre 0 e 1, e apenas 5% caem abaixo da linha. Nos painéis seguintes, o menor valor de p da família se acumula progressivamente à esquerda, e com vinte testes quase toda a distribuição está abaixo de 0,05. O mesmo raciocínio vale para quem testa vários nutrientes, várias espécies ou várias praias e relata só o que se destacou. Quanto mais coisas se olha, mais fácil encontrar alguma.
...
A ordem importa, vindo primeiro a pergunta global e depois as comparações específicas. E é justamente para essa segunda etapa que existem procedimentos capazes de considerar quantas comparações estão sendo feitas.
...
Em desenho balanceado com $n_g$ unidades por grupo e quadrado médio residual $QM_e$, seu erro padrão é

$$
s_C=\sqrt{QM_e\sum_{i=1}^{k}\frac{c_i^2}{n_g}}.
$$
...
Os três valores originais foram multiplicados por 3, o número de comparações. O primeiro par continua abaixo de 0,05 depois do ajuste, enquanto o segundo, que parecia significativo, deixa de ser.
...
Quinn e Keough [@quinn2002] dedicam um capítulo às comparações múltiplas, com a comparação detalhada entre os procedimentos citados aqui e outros métodos disponíveis.
:::
